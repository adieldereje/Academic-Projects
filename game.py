...
class Game:
    ...
    def process_eat(self, words): #NEWSTUFF
        if len(words) == 1:
            print('You have to tell me what you want to eat.')
        elif words[1] not in self.items.items_dict:
            print('There is nothing to eat.')
        elif words[1] not in self.player.inventory:
            print("You're not carrying that.")
        elif words[1] != 'orange' or 'cockroach':
            print("you can't eat that")
        elif words[1]== 'cockroach':
            print ('you ate the cockroach, like the weirdo you are')
            self.inventory.remove (words[1])
            #self.player.health -= 10
            print ('your player health dropped to',self.player.health_after_eating('cockroach'))
        else:
            print('You eat the orange, its so yummy!')
            self.player.inventory.remove(words[1])
            #self.player.health += 10
            print('Your health increased to',self.player.health_after_eating('orange'))
    ...
    def process_take(self, words):
        if len(words) == 1:
        ...
        elif words[1] in self.player.inventory:
            ...
            if item.name in room.items_in_room:
                ...
                elif self.items.get_total_weight(self.player.inventory) + item.weight > self.player.max_capacity: #NEWSTUFF
                ...
                for item_name_in_room in room.items_in_room:
                    if item_name_in_room in self.containers:
                        ...
                        if item.name in container.contents and container.status == 'open':
                            if self.items.get_total_weight(self.player.inventory) + item.weight > self.player.max_capacity: #NEWSTUFF
                                print("You can't carry it with your current load.")
                            else:
                                self.player.inventory.add(item.name)
    ...
    def process_examine(self, words):
        if len(words) == 1:
            ...
            item = self.items.get_item(words[1])
            room = self.rooms.get_room(self.current_room)
            if item.name not in room.items_in_room and item.name not in self.player.inventory:
                print("I don't see that here.")
            else:
                print(item)
    ...
    def process_open(self, words):
        ...
        if len(words) == 1:
            ...
            print("I don't know what that is.")
        elif words[1] not in room.items_in_room and words[1] not in self.player.inventory:
            print("I don't see that here.")
    ...
    def process_put(self, words):
        ...
        elif words[3] not in room.items_in_room and words[3] not in self.player.inventory:
    ...
    def process_read(self, words):
        ...
        if len(words) == 1:
            ...
            if 'book' not in self.player.inventory:
